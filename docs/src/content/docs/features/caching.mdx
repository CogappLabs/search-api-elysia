...
All JSON responses are gzip-compressed when the client sends an `Accept-Encoding: gzip` header (which browsers and most HTTP clients do by default). This typically reduces response sizes by 70-80%.
