...
- **Elasticsearch**: Supports `apiKey` or `username`/`password` auth. Full feature support including histograms, geo grid, and phrase suggestions.
- **OpenSearch**: Uses the same query DSL as Elasticsearch. Supports `username`/`password` auth only (the OpenSearch JS client does not support `apiKey` auth). Full feature support.
- **Meilisearch**: Uses `apiKey` for auth. Filters use Meilisearch string syntax (translated automatically). Histograms, geo grid, and phrase suggestions are not supported and return empty/default values. Boosts and `searchableFields` are managed at the Meilisearch index level, not at query time.
