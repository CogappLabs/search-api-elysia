...
The API uses an engine abstraction that makes it straightforward to add new search backends. Three engines are currently implemented: Elasticsearch, OpenSearch, and Meilisearch.

## Architecture

Elasticsearch and OpenSearch share a base class (`ElasticCompatEngine`) since they use an identical query DSL. Subclasses only differ in client construction and response unwrapping. Meilisearch implements `SearchEngine` directly because its API model is fundamentally different.

```
SearchEngine (interface)
├── ElasticCompatEngine (abstract)
│   ├── ElasticsearchEngine
│   └── OpenSearchEngine
└── MeilisearchEngine
```
...
- The `esField` config property name is ES-specific but the alias mechanism operates at the route layer before any engine is called. It works identically for all engines.
