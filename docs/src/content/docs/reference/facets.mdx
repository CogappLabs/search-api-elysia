...
:::note
The facets endpoint only supports string and string array filters — not boolean or range filters. For those filter types, use the [search endpoint](/reference/search/).
:::
...
curl "http://localhost:3000/collections/facets/country"

# Search for countries matching "sc"
curl "http://localhost:3000/collections/facets/country?q=sc"

# Top 5 regions
curl "http://localhost:3000/collections/facets/region?maxValues=5"

# Regions within Scotland only
curl "http://localhost:3000/collections/facets/region?filters=%7B%22country%22%3A%22Scotland%22%7D"
```
